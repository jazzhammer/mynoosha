...
<script lang="ts">
  ...
  let retrieveAgreementTimeout: any = null;

  function retrieveAgreements(): void {
    if (retrieveAgreementTimeout) {
      clearTimeout(retrieveAgreementTimeout);
    }
    retrieveAgreementTimeout = setTimeout(
      () => {
        if (ymdFrom && ymdThrough && ymdFrom.getTime() < ymdThrough.getTime()) {
          ...
          // console.log(`invalid agreement search interval:`);
  ...
  function selectAgreement(selected: Agreement): void {
    AgreementStore.set({
      type: crud.READ,
      payload: selected
    })
...
<div class="new-agreement flex flex-col border-myroon-100 border p-3 mr-6 ml-3 rounded w-full text-myhigh_white"
     style="min-width: 226px; max-width: 375px; font-size: 10pt;" data-testid="browse_agreement"
>
  <div class="bg-mywood-900 rounded mb-5 w-full" data-testid="browse_agreement_header" id="browse_agreement_header">browse agreements</div>
  <div class="search-bar text-mywood-900 flex flex-row" data-testid="browse_agreement_form">
    <div class="" data-testid="browse_agreement_name">from</div>
    <div>
    ...
    <div class="" data-testid="browse_agreement_name">through</div>
    <div>
