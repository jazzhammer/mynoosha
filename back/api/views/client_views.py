...
def clients(request, *args, **kwargs):
    if request.method == 'GET':
        return getClients(request, *args, **kwargs)
    elif request.method == 'POST':
        return postClients(request, *args, **kwargs)
    elif request.method == 'DELETE':
        return deleteClients(request, *args, **kwargs)
    else:
        return JsonResponse({'data': f"{request.method} unsupported"}, status=400, safe=False)


def getClients(request, *args, **kwargs):
...
def postClients(request, *args, **kwargs):
...
def deleteClients(request, *args, **kwargs):
