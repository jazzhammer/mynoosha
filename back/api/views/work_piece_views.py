...
def work_pieces(request):
    ...
    if request.method == 'GET':
        founds = WorkPiece.objects.all().order_by('start_utcms')
        dt_filtered = False
        if pre_start:
        ...
        if dt_filtered:
            ...
            except WorkPiece.DoesNotExist:
                return JsonResponse({'error': f'WorkPiece[{client_id=}] not found'}, status=404, safe=False)
        else:
